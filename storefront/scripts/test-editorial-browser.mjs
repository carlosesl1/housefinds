...
async function waitFont(page, selector, fragment) {
  const report = await page.locator(selector).first().evaluate(async el => {
    ...
    const faces = [...document.fonts].filter(face => clean(face.family) === clean(family) && face.status === 'loaded').map(face => ({ family: face.family, status: face.status, weight: face.weight }))
    return { computed: style.fontFamily, faces }
  })
  assert.ok(report.computed.toLowerCase().includes(fragment), JSON.stringify(report))
  assert.ok(report.faces.length, `Fallback-only font: ${JSON.stringify(report)}`)
  return report
}
async function checkEditorialScale(page) {
  const report = await page.locator('.hf-editorial-scope').evaluate(async scope => {
    ...
    await settle()
    const tuned = sizes(titles), uiTuned = sizes(ui)
    ...
    await settle()
    const original = sizes(titles), uiOriginal = sizes(ui)
    ...
    }).map(el => ({ text: el.textContent.trim(), rect: el.getBoundingClientRect().toJSON(), client: el.clientWidth, scroll: el.scrollWidth }))
  ...
  assert.ok(report.headings.length)
  for (const h of report.headings) assert.ok(Math.abs(h.tuned - h.original * .9) < .05, `Wrong Lora size: ${JSON.stringify(h)}`)
...
try {
  for (const width of [320, 360, 390, 540, 640, 767, 768, 1024, 1099, 1100, 1440, 1920]) {
    ...
    for (const banner of await page.locator('[data-promotion]').all()) {
      ...
      const g = await banner.evaluate(el => {
        const outer = el.getBoundingClientRect(), copy = el.querySelector('[data-campaign-layer="content"]').getBoundingClientRect(), art = el.querySelector('[data-campaign-layer="image"]').getBoundingClientRect()
        const failures = []
        ...
        return { failures, below: art.top >= copy.bottom - 1, src: img.currentSrc, loaded: img.naturalWidth > 0, id: el.dataset.promotion, font: getComputedStyle(el.querySelector('h3')).fontFamily }
      })
      assert.deepEqual(g.failures, [], `Overflow at ${width}: ${g.id}`)
      assert.ok(g.loaded)
      assert.ok(g.font.toLowerCase().includes('lora'))
      assert.equal(decodeURIComponent(g.src).includes('/mobile/'), width < 768)
      if (width < 768) assert.ok(g.below, `Art covers text at ${width}`)
      await banner.focus()
    ...
    const dark = await page.locator('#featured-find .hf-eyebrow').evaluate(el => ({ color: getComputedStyle(el).color, background: getComputedStyle(el.closest('section').firstElementChild).backgroundColor }))
    const fg = dark.color.match(/[\d.]+/g).slice(0, 3).map(Number), bg = dark.background.match(/[\d.]+/g).slice(0, 3).map(Number)
    ...
    checks.push({ name: `Home ${width}px`, result: 'pass', editorial: heroFont.computed, headingSizes, darkEyebrowContrast: Number(ratio.toFixed(2)) })
    await context.close()
  }
  ...
  if (activePage && !activePage.isClosed()) {
    const diagnostic = await activePage.evaluate(() => ({ viewport: innerWidth, scrollWidth: document.documentElement.scrollWidth, overflowing: [...document.querySelectorAll('body *')].filter(el => { const r = el.getBoundingClientRect(); return r.width && (r.right > innerWidth + 1 || r.left < -1) }).slice(0, 35).map(el => ({ tag: el.tagName, class: String(el.className), rect: el.getBoundingClientRect().toJSON(), text: el.textContent.slice(0, 80) })) }))
    await fs.writeFile(path.join(output, `failure-${activeWidth}.json`), JSON.stringify(diagnostic, null, 2))
    await activePage.screenshot({ path: path.join(output, `failure-${activeWidth}.png`) })
    console.log('LAYOUT_DIAGNOSTIC', JSON.stringify(diagnostic))
  }
} finally {
  await browser.close()
