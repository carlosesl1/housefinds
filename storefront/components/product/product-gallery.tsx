...
export function ProductGallery({ images, productName }: { images: StorefrontImage[]; productName: string }) {
  ...
  return (
    <>
      <div className="relative aspect-[4/3] touch-pan-y overflow-hidden rounded-[var(--hf-radius-lg)] border border-black/[.05] bg-[#efede7] shadow-[0_18px_56px_rgba(29,42,34,.055)]" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
        ...
        {gallery.length > 1 && (
          <>
            <button type="button" onClick={(event) => { event.stopPropagation(); previous() }} className="hf-icon-button absolute left-3 top-1/2 z-30 hidden -translate-y-1/2 sm:grid" aria-label="Previous product image"><ChevronLeftIcon className="size-5" /></button>
            <button type="button" onClick={(event) => { event.stopPropagation(); next() }} className="hf-icon-button absolute right-3 top-1/2 z-30 hidden -translate-y-1/2 sm:grid" aria-label="Next product image"><ChevronRightIcon className="size-5" /></button>
            <div className="pointer-events-none absolute bottom-3 left-1/2 z-20 flex -translate-x-1/2 gap-1.5 rounded-full bg-black/20 px-2.5 py-2 backdrop-blur sm:hidden" aria-hidden="true">
      ...
      {gallery.length > 1 && (
        <div className="mt-3 hidden items-stretch gap-2 sm:flex" aria-label="Product image thumbnails">
          ...
          {remaining > 0 && (
            <button type="button" onClick={() => setExpanded(true)} className="grid min-w-[84px] place-items-center rounded-[var(--hf-radius-md)] border border-black/[.08] bg-white px-3 text-center text-xs font-semibold text-black/55 transition hover:border-[var(--hf-brand-muted)] hover:bg-[var(--hf-brand-soft)] hover:text-[var(--hf-brand)]" aria-label={`View all ${gallery.length} product images`}>
      ...
      {expanded && (
        <div data-product-gallery-dialog className="fixed inset-0 z-[160] bg-[#0d100e]/96 p-3 text-white sm:p-6" role="dialog" aria-modal="true" aria-label={`${productName} image viewer`}>
          <div className="relative mx-auto flex h-full max-w-[1500px] flex-col">
            ...
            <div className="relative min-h-0 flex-1 touch-pan-y overflow-hidden rounded-[var(--hf-radius-lg)] bg-black/20" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
