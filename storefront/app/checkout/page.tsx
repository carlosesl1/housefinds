...
function Field({ label, value, onChange, placeholder, type = 'text', autoComplete, className = '' }: {
  label: string
  value: string
...
export default function CheckoutPage() {
  ...
  const addressReady = Boolean(
    address.email?.trim() && address.first_name.trim() && address.last_name.trim() &&
    address.address_1.trim() && address.city.trim() && address.postcode.trim(),
  )
  const paymentReady = Boolean(
    addressReady && (!cart?.needs_shipping || (cart.has_calculated_shipping && selectedShipping)),
  )
    ...
    return (
      <main className="min-h-[70vh] bg-[#fbfaf7] px-5 py-20 lg:px-8">
        <div className="mx-auto max-w-2xl rounded-[34px] border border-black/[.06] bg-white p-8 text-center shadow-[0_22px_80px_rgba(34,45,37,.05)] sm:p-12">
          ...
          <Link href="/shop" className="mt-8 inline-flex h-13 items-center gap-2 rounded-full bg-[#355f4a] px-6 font-semibold text-white"><ArrowLeftIcon className="size-4" /> Browse products</Link>
        </div>
      </main>
    )
  }
    ...
    <main className="bg-[#f5f4ef] px-5 py-10 lg:px-8 lg:py-16">
      <div className="mx-auto max-w-[1380px]">
        <div className="mb-8 flex items-center justify-between gap-4">
          <Link href="/shop" className="inline-flex items-center gap-2 text-sm font-medium text-black/48 transition hover:text-black"><ArrowLeftIcon className="size-4" /> Continue shopping</Link>
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_420px] xl:gap-12">
          <section className="space-y-5">
            <div className="rounded-[32px] border border-black/[.06] bg-white p-6 shadow-[0_20px_70px_rgba(34,45,37,.04)] sm:p-8">
              <div className="flex items-start justify-between gap-6">
                <div>
                  ...
                  <p className="mt-3 max-w-xl text-sm leading-6 text-black/48">Enter the address where you want the order delivered. WooCommerce will calculate the available UK shipping methods.</p>
                </div>
              ...
              {deliveryAttempted && !addressReady && <p className="mt-4 rounded-2xl bg-amber-50 px-4 py-3 text-sm text-amber-900">Please complete email, name, address, city and postcode before calculating delivery.</p>}
              <button type="button" disabled={loading} onClick={() => void calculateDelivery()} className="mt-7 inline-flex h-13 items-center justify-center rounded-full bg-[#355f4a] px-7 font-semibold text-white transition hover:bg-[#294b3a] disabled:opacity-50">
                ...
                <div className="mt-5 space-y-3">
                  {shippingRates.map((rate) => (
                    <button type="button" key={`${rate.packageId}-${rate.rate_id}`} onClick={() => void selectShipping(rate.packageId, rate.rate_id)} disabled={loading} className={`flex w-full items-center justify-between gap-5 rounded-2xl border p-4 text-left transition ${rate.selected ? 'border-[#557562] bg-[#edf3ee]' : 'border-black/10 bg-white hover:border-black/20'}`}>
                      <div><p className="font-semibold text-[#172018]">{rate.name}</p>{rate.delivery_time && <p className="mt-1 text-sm text-black/45">{rate.delivery_time}</p>}{rate.description && <p className="mt-1 text-xs text-black/38">{rate.description}</p>}</div>
                      <div className="flex items-center gap-3"><strong>{money(rate.price)}</strong>{rate.selected && <CheckCircleIcon className="size-5 text-[#456b55]" />}</div>
                    </button>
                  ))}
                </div>
              ) : (
                <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">WooCommerce did not return a shipping method for this address. Check the UK shipping zone and rates in WooCommerce.</div>
              )}
            </div>
            ...
            <div className="rounded-[32px] border border-black/[.06] bg-white p-6 shadow-[0_20px_70px_rgba(34,45,37,.04)] sm:p-8">
              ...
              <p className="mt-2 text-sm leading-6 text-black/48">Card details are handled securely by Stripe. WooCommerce remains the system of record for the order.</p>
              {!paymentReady && <div className="mt-5 rounded-2xl bg-[#f5f5f1] p-4 text-sm leading-6 text-black/48">Complete your delivery details{cart.needs_shipping ? ' and choose a shipping method' : ''} to unlock payment.</div>}
          ...
          <aside className="h-fit lg:sticky lg:top-28">
            <div className="rounded-[32px] border border-black/[.06] bg-white p-6 shadow-[0_22px_80px_rgba(34,45,37,.055)]">
              ...
              <div className="mt-6 border-t border-black/[.07] pt-5">
                <div className="flex gap-2"><input value={coupon} onChange={(event) => setCoupon(event.target.value)} placeholder="Discount code" className="h-12 min-w-0 flex-1 rounded-full border border-black/10 px-4 text-sm outline-none focus:border-[#557562]" /><button type="button" disabled={loading || !coupon.trim()} onClick={() => void applyCoupon(coupon).then((ok) => ok && setCoupon(''))} className="h-12 rounded-full bg-[#172018] px-5 text-sm font-semibold text-white disabled:opacity-40">Apply</button></div>
              ...
              <div className="mt-6 flex items-center gap-2 rounded-2xl bg-[#f0f2ed] p-4 text-xs leading-5 text-black/48"><LockClosedIcon className="size-4 shrink-0 text-[#456b55]" /> Cart, delivery and order totals are calculated by WooCommerce.</div>
            </div>
          </aside>
        </div>
      </div>
