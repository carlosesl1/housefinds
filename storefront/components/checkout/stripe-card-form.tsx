...
type StripeInstance = {
  ...
  createPaymentMethod: (data: Record<string, unknown>) => Promise<{
  ...
  const submitPayment = async () => {
    ...
    try {
      ...
      if (paymentMethodResult.error || !paymentMethodResult.paymentMethod) {
        throw new Error(paymentMethodResult.error?.message || 'Your card details could not be validated.')
      }
      ...
      if (!checkoutResponse.ok) {
        throw new Error(checkoutErrorMessage(text, `Payment could not be processed (${checkoutResponse.status}).`))
      }
      ...
      if (paymentResult.payment_status === 'failure' || paymentResult.payment_status === 'failed') {
        throw new Error(details.errorMessage || 'Stripe could not process this payment.')
      }
      ...
      if (checkout.order_id) {
        window.location.assign(`/order-confirmation?order=${encodeURIComponent(String(checkout.order_id))}`)
        return
      }

      throw new Error('The payment was accepted but no order redirect was returned. Please contact support before trying again.')
    } catch (reason) {
  ...
  if (!publishableKey) {
    return (
      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-950">
        <strong>Stripe is connected in WooCommerce.</strong>
        <p className="mt-1">The storefront still needs the Stripe publishable key in Vercel before card fields can be displayed. No secret key is required here.</p>
      </div>
    )
  }
    ...
    <div>
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <span className="grid size-11 place-items-center rounded-2xl bg-[#e7eee9] text-[#456b55]"><CreditCardIcon className="size-5" /></span>
          <div><p className="font-semibold">Credit or debit card</p><p className="text-xs text-black/42">Securely processed by Stripe</p></div>
        </div>
